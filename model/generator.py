...
from model.utils import lerp
from layers.upfirdn_2d import setup_resample_kernel, upsample_2d
from layers.custom_layers import LabelEmbedding, Dense, BiasAct, LeakyReLU, Noise, FusedModConv
...
class Mapping(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        # embed label
        x = self.labels_embedding(inputs)
...
class Generator(tf.keras.Model):
    ...
    def style_mixing_regularization(self, labels, w_broadcasted1):
        # get another w and broadcast it
        #labels2 = tf.random.normal(shape=tf.shape(labels1), dtype=tf.dtypes.float32)
        # 같은 keypoint, 다른 화자인 새로운 labels를 생성
        depth = 5
        indices = tf.random.uniform((tf.shape(labels)[0],), 0, depth, dtype=tf.int32)
        one_hot = tf.one_hot(indices, depth, dtype=tf.dtypes.float32)
        labels = tf.concat([labels[:, :254], one_hot], axis=1)
        #print(labels)
        dlatents2 = self.g_mapping(labels)
    ...
    def call(self, inputs, ret_w_broadcasted=False, truncation_psi=1.0, truncation_cutoff=None, training=None, mask=None):
        ...
        dlatents = self.g_mapping(labels)
        w_broadcasted = self.broadcast(dlatents)

        if training:
            self.update_moving_average_of_w(w_broadcasted)
            w_broadcasted = self.style_mixing_regularization(labels, w_broadcasted)
