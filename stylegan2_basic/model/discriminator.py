...
from layers.custom_layers import Dense, ResizeConv2D, BiasAct, LeakyReLU, MinibatchStd
...
class Discriminator(tf.keras.Model):
    def __init__(self, d_params, **kwargs):
        ...
        # set last dense layer -> real/fake
        self.last_dense = Dense(1, gain=1.0, lrmul=1.0, name='last_dense')
        self.last_bias = BiasAct(lrmul=1.0, act='linear', name='last_bias')

        # set last auxiliary dense layer -> labels
        self.last_aux_dense = Dense(self.labels_dim, gain=1.0, lrmul=1.0, name='last_aux_dense')
        self.last_aux_bias = BiasAct(lrmul=1.0, act='linear', name='last_aux_bias')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        real_fake = self.last_bias(self.last_dense(x))
        auxiliary = self.last_aux_bias(self.last_aux_dense(x))

        return real_fake, auxiliary
